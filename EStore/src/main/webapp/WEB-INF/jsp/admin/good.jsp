			...
			<div class="right-nav">
					<ul>
							<li><img src="${root}/image/home.png"></li>
								...
								<li><a href="#">最新公告</a></li>
								<li>></li>
								<li><a href="${root}/item/ItemStatus.html">下架商品列表</a></li>
								<li>></li>
								<li><a href="${root}/item/addItems.do">添加商品</a></li>
						</ul>
			</div>
			<div class="main">
...
//显示第一页商品
	$(function(){
		...
		$.ajax({
			"url":"${root}/item/data/findItemByCount5.do",
			"type":"post",
			"dataType":"JSON",
				...
				if(code==0){
					...
					//	alert("循环遍历"+cartList.id);
						$("<tr></tr>").attr("id",item.id).attr("named","liPage").
			    		 ...
			    		 append($("<td width='50'></td>").html($("<a href='javascript:void(0)' onclick='deleteItem("+item.id+")'>下架该商品</a>")))
			    		.appendTo("#table")
...
//分页操作
	$(function(){
		$.ajax({
			"url":"${root}/item/data/findCount5.do",
			"type":"post",
			"dataType":"json",
			success:function(result){
				var code = result.error_code;
...
<script >
 // 下一页功能
	function goNextPage(){
	   ...
	   $.ajax({
		   "url":"${root}/item/data/downPage1.do",
			"type":"post",
			"dataType":"json",
			"data":{"pageIndex":pageIndex},
			success:function(result){
