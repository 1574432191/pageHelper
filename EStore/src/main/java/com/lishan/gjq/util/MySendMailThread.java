...
public class MySendMailThread extends Thread {
	...
	public MySendMailThread(User user) {
		System.out.println("邮箱发送工具类"+user+user.getUsername());
		this.user = user;
	}
	...
	public void run() {
		...
		Session session = Session.getDefaultInstance(p, new Authenticator() {
			...
			protected PasswordAuthentication getPasswordAuthentication() {
				...
				PasswordAuthentication pa = new PasswordAuthentication(
						"lfzhang_7@163.com", "zlf754389736");
				// 后面的字符是授权码，用qq密码不行！！
				return pa;
			}
		});
		...
		try {
			...
			// 邮件信息封装
			// 1发件人
			msg.setFrom(new InternetAddress("lfzhang_7@163.com"));
			// 2收件人
			...
			sbd.append("<font color='red'><a href='http://localhost:8080/EStore/user/emailActive.do?username="
					+ user.getUsername() + "' target='_blank'");
			sbd.append(">立即激活</a></font><br/>");
			sbd.append("或者点击下面链接:<br/>");
			sbd.append("http://localhost:8080/EStore/user/emailActive.do?username="
					+ user.getUsername() + "<br/>");
